...
def doPlot(rundir,colorbyfeaturename,cap1,cap2):
    mat = scipy.io.loadmat(rundir+"/results.mat")
    ...
    for x in xrange(0,len(mat["FIcell"][0])):
        allimp.append([])
        fo = open(rundir+"/topFs."+str(x)+".tsv","w")
    ...
    #print mat["FIcell"][0]
    try:
        fo = open(rundir+"/leaffile.cutoff.0.0.json")
    ...
    fo.write(vispage%{"json":json.dumps(results),"imp":json.dumps(clinimp[:10]+imp[:10]),"clinimp":json.dumps(clinimp[:16]),"leafdata":json.dumps(leafdata),"colorbyfeaturename":colorbyfeaturename,"cap1":cap1,"cap2":cap2})
    fo.close()
